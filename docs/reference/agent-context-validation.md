...
**Recommendation:** proceed with WSL rollout. The accuracy preservation
result is the load-bearing finding; the token ratio improves with scale.

**Update — WSL replication done.** The experiment has since been repeated
on a real legacy capability (WSL Android `video-playback`, ~11.6K LOC).
At that scale the ratio is **20x** for best-case curated raw code and
**42x** for the whole capability, with `agent-context.md` at **100%**
accuracy versus 92% for curated raw code. The 1.6x above was a
small-capability artifact; the projection holds. See
[WSL replication](#wsl-replication-video-playback).
...
| Medium (WSL Android video-playback) | 5 fragments + shared helpers | ~10K LOC | ~50-80K | ~3K | **~15-25x** |
...
## WSL replication (video-playback)

This section reports a second run of the experiment on a **real legacy
capability** — the WSL Android `video-playback` feature — performed as a
local feasibility test. It exists to pressure-test the scale projection
above with measured data and to apply the three methodology changes the
PoC prescribed for WSL.

The capability is the medium-scale bucket from the projection table:
~11.6K LOC across five host fragments, a 2,118-line host Activity, and
three player backends. Its `agent-context.md` was generated by the
`/docs-bootstrap` skill (not hand-tuned for the experiment), so what is
under test is the artifact the system actually produces.

### What changed from the PoC

1. **Raw code is no longer a single curated set.** We measure two
   raw-code conditions instead of one:
   - **B_curated** — the minimal correct file set the
     `agent-context.md` read-order points to (helper + interfaces +
     `WslExoPlayer` + one representative host fragment + `SingleActivity`).
     This is the *best case* for raw code: the agent already knows which
     files matter. Analogous to the PoC's pre-curated B.
   - **B_full** — every file in the capability (all five host fragments,
     all backends, helpers, views). This is the *no-map* case: read the
     whole capability because nothing told you which 3 files matter. A
     proxy for unguided search overhead.
2. **The judge is a different model from the answerer.** Answerer:
   `claude-sonnet-4-6`. Judge: `claude-opus-4-8`. This removes the
   same-model cross-eval bias the PoC flagged as a limitation.
3. **A discovery-category question** (Q11) was added, which rewards
   knowing *where* code lives — the search-elimination value directly.
...
question (Q12) scored 2/2 in every condition — no fabrication.
...
1. **The 1.6x PoC ratio was a small-capability artifact.** At realistic
   ...
   thousands of lines. The PoC's `agent-orchestration` capability was
   ~370 LOC total; here a single host fragment is larger than that.
2. **Accuracy favors `agent-context.md`, more clearly than in the PoC.**
   A scored 100%; B_curated scored 92%, missing one gotcha (Q06: why the
   fullscreen player is invisible to `uiautomator` dumps — it lives in a
   separate Dialog window) and one common-change (Q09: enumerate all five
   host fragments to touch). Both misses are exactly the operational
   hindsight the `gotchas` and `common_changes` fields capture and that
   raw code, even when present, does not surface on its own.
3. **More raw code recovers accuracy, but at 42x cost.** B_full reached
   100% — the missing fragments and the Dialog code were all in context —
   but at 126K tokens, **approaching the 200K context window**. One step
   up in capability size (the projection's ~30K LOC bucket) would exceed
   it, which is the projection's "raw-code budget exceeds practical
   context windows" prediction landing in practice.

This measured 20x (curated) to 42x (full) sits squarely in — and at the
top of — the projection table's **~15-25x** estimate for this scale
bucket, now backed by data rather than a growth model.

### Honest caveats specific to this run

- **B_curated includes only one representative host fragment.** That is
  faithful to the read-order (the four skeleton fragments are near-exact
  copies), but it is part of why B_curated missed Q09. A curated set with
  all five fragments would likely score 2 on Q09 — at a higher token cost,
...
- **B_full is an upper bound, not literal agentic search.** It reads the
  ...
  and B_full. We did not build the agentic-search harness for this
  feasibility pass; it remains the most faithful future measurement.
- **One capability, one run, same judge caveat partially addressed.**
  The judge is now a different model, but it is still a single judge and a
  single capability. The relative comparison across conditions is more
  robust than the absolute scores.

Replication: `scripts/agent-context-eval-wsl/run-wsl.ts` (reads the WSL
repo's raw source by absolute path; writes nothing into it). Inputs:
`scripts/agent-context-eval-wsl/questions.json` and
`scripts/agent-context-eval-wsl/video-playback-agent-context.md` — the
artifact under test, generated by `/docs-bootstrap` against the WSL repo
and vendored here so the experiment is self-contained. The full
per-question run (every response and the judge's reasoning) is saved at
`scripts/agent-context-eval-wsl/results-wsl-2026-06-01T14-12-38-966Z.json`.

> Note: the WSL repo itself was left untouched — the bootstrap artifacts
> generated there during the feasibility test were removed after the
> `agent-context.md` was vendored into this repo.
...
**Proceed with WSL rollout.** Two reasons:
...
**Status: done (partially), see [WSL replication](#wsl-replication-video-playback).**
(1) was addressed by the two-condition B_curated / B_full split — a whole-
capability upper bound rather than a literal grep loop, which remains
future work. (2) was addressed (answerer `claude-sonnet-4-6`, judge
`claude-opus-4-8`). (3) was **not** done: the WSL run used a single
capability, so cross-capability questions were replaced by a discovery
category. Documenting a second WSL capability would let the `cross_refs`
question type be tested.
